...
export async function renderWebdevRound(container, params, search = {}) {
  ...
  container.innerHTML = `
    ...
    <main class="min-h-[calc(100vh-76px)] relative pb-24">
      ...
      <div class="max-w-5xl mx-auto px-6 pt-12 relative z-10">
        <div class="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div class="space-y-2">
            <div class="flex items-center gap-3 mb-2">
              <span class="px-3 py-1 bg-primary/20 text-primary rounded-full text-[10px] font-bold uppercase tracking-widest">Round ${round.round_number}</span>
              ${isLocked ? `<span class="px-3 py-1 bg-secondary/20 text-secondary rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5"><span class="material-symbols-outlined text-[14px]">beenhere</span> Submitted</span>` : ''}
            </div>
            <h1 class="text-4xl md:text-5xl font-bold font-headline tracking-tighter text-white">Project Submission</h1>
            <p class="text-on-surface-variant max-w-md">${round.title}</p>
          </div>
          
          <div class="flex flex-col items-end gap-2">
            <span class="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold">Remaining Time</span>
            <div id="webdev-timer" class="flex items-center gap-4 bg-surface-container-low border border-outline-variant/20 px-6 py-3 rounded-2xl shadow-lg">
              <span class="material-symbols-outlined text-primary text-xl">timer</span>
              <span class="text-2xl font-headline font-bold text-white tabular-nums">${Timer.formatTime(round.duration_minutes * 60 * 1000)}</span>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
          <div class="md:col-span-12 lg:col-span-8 space-y-6">
            ...
            <!-- INPUT FORM -->
            <div class="glass-panel p-8 rounded-2xl space-y-8 ${isLocked ? 'opacity-50 pointer-events-none' : ''}">
              <div class="space-y-6">
                <h2 class="text-lg font-headline font-bold text-primary flex items-center gap-2">
                  <span class="material-symbols-outlined">rocket_launch</span> Deployment Status
                </h2>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <label class="relative flex items-center p-4 rounded-xl bg-surface-container-lowest border border-outline-variant/20 cursor-pointer group hover:bg-white/5 transition-all">
                    <input type="radio" name="deployed" value="true" class="hidden peer" ${existing?.is_deployed ? 'checked' : ''}>
                    <div class="w-5 h-5 rounded-full border-2 border-outline-variant mr-3 flex items-center justify-center peer-checked:border-primary peer-checked:bg-primary transition-all">
                      <div class="w-2 h-2 rounded-full bg-surface-container-lowest scale-0 peer-checked:scale-100 transition-transform"></div>
                    </div>
                    <div>
                      <span class="text-white font-bold block">Live Project</span>
                      <span class="text-[10px] text-on-surface-variant uppercase">I have a deployed URL</span>
                    </div>
                  </label>
                  ...
                  <label class="relative flex items-center p-4 rounded-xl bg-surface-container-lowest border border-outline-variant/20 cursor-pointer group hover:bg-white/5 transition-all">
                    ...
                    <div>
                      <span class="text-white font-bold block">Code Only</span>
                      <span class="text-[10px] text-on-surface-variant uppercase">Not deployed yet</span>
                    </div>
                  </label>
                </div>
              </div>

              <div id="submission-links-section" class="space-y-6 pt-6 border-t border-outline-variant/10 ${existing?.is_deployed ? '' : 'hidden'}">
                <h2 class="text-lg font-headline font-bold text-primary flex items-center gap-2">
                  <span class="material-symbols-outlined">link</span> Submission Links
                </h2>
                
                <div class="space-y-6">
                  <div class="space-y-2">
                    <label class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block">Source Code (GitHub/GitLab)</label>
                    <div class="relative">
                      <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline/50">code</span>
                      <input id="github-link" class="w-full bg-surface-container-lowest border-none rounded-xl py-4 pl-12 pr-4 text-white placeholder:text-outline/20 font-body" placeholder="https://github.com/team/project" value="${existing?.github_link || ''}" />
                    </div>
                  </div>

                  <div id="live-url-container" class="space-y-2">
                    <label class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block">Deployment / Live URL</label>
                    <div class="relative">
                      <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline/50">public</span>
                      <input id="live-link" class="w-full bg-surface-container-lowest border-none rounded-xl py-4 pl-12 pr-4 text-white placeholder:text-outline/20 font-body" placeholder="https://project.vercel.app" value="${existing?.live_link || ''}" />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="md:col-span-12 lg:col-span-4 space-y-6">
            <!-- INSTRUCTIONS CARD -->
            <div class="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/10 shadow-xl">
              <h3 class="text-white font-headline font-bold text-lg mb-6 flex items-center gap-2">
                <span class="material-symbols-outlined text-primary">description</span> 
                Round Guidelines
              </h3>
              
              <div class="prose prose-invert prose-sm max-w-none text-on-surface-variant leading-relaxed">
                ${guidelines ? guidelines.replace(/\n/g, '<br>') : `
                  <ul class="space-y-4 list-none p-0">
                    <li class="flex gap-3"><span class="material-symbols-outlined text-primary text-sm mt-0.5">verified</span><p>Must be a working web application.</p></li>
                    <li class="flex gap-3"><span class="material-symbols-outlined text-primary text-sm mt-0.5">verified</span><p>GitHub repository must be public or shared.</p></li>
                    <li class="flex gap-3"><span class="material-symbols-outlined text-primary text-sm mt-0.5">verified</span><p>Finalize before the timer hits zero for auto-save.</p></li>
                  </ul>
                `}
              </div>
              ...
              ${!isLocked ? `
                <div class="mt-8 p-4 rounded-xl bg-primary/5 border border-primary/10">
                  <div class="flex gap-3">
                    ...
                    <p class="text-[10px] text-primary/80 uppercase font-bold tracking-wider leading-normal">Finalizing will lock your submission. You cannot make any changes after clicking "Finalize".</p>
                  </div>
                </div>
              ` : ''}
            </div>
